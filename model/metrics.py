...
class Seq2SeqSpanMetric(MetricBase):
    ...
    def evaluate(self, target_span, pred, tgt_tokens, region_pred,region_label,cover_flag,predict_mode = False):
        ...
        for i, (ts, ps) in enumerate(zip(target_span, pred.tolist())):
            ...
            mner_tp,mner_fp,mner_fn= _compute_tp_fn_fp_mner(all_pairs, all_ts,self.region_num)
            eeg_tp,eeg_fp,eeg_fn= _compute_tp_fn_fp_eeg(all_pairs, all_ts,self.region_num)
            if self.print_mode:
            ...
            self.mner_tp+= mner_tp
            self.mner_fp+= mner_fp
            self.mner_fn+= mner_fn
    ...
    def get_metric(self, reset=True):
        ...
        mner_f, mner_p, mner_r = _compute_f_pre_rec(1, self.mner_tp, self.mner_fn, self.mner_fp)
        eeg_f,eeg_p,eeg_r = _compute_f_pre_rec(1, self.eeg_tp, self.eeg_fp, self.eeg_fn)

        res['f'] = round(f*100, 2)
        res['rec'] = round(rec*100, 2)
        res['pre'] = round(pre*100, 2)
        res['em'] = round(self.em/self.total, 4) #完全序列正确。
        res['uc'] =round(self.uc)
        res['nc'] =round(self.nc)
        res['tc'] =round(self.tc)
        res['sc'] =round(self.sc)
        res['mner_f']=round(mner_f*100, 2)
        res['mner_p']=round(mner_p*100, 2)
        res['mner_r']=round(mner_r*100, 2)
        res['eeg_f']=round(eeg_f*100, 2)
        res['eeg_p']=round(eeg_p*100, 2)
        res['eeg_r']=round(eeg_r*100, 2)
        if reset:
            self.total = 0
            self.fp = 0
            self.tp = 0
            self.fn = 0
            self.mner_tp, self.mner_fn, self.mner_fp=0,0,0
            self.eeg_tp, self.eeg_fp, self.eeg_fn=0,0,0
            self.em = 0
            self.uc =0
            self.nc =0
            self.tc =0
...
def _compute_tp_fn_fp(ps, ts,region_num):
    ...
    supports = len(ts) #t : true
    pred_sum = len(ps) # p : pred
    correct_num = 0
    useful_correct = 0
    noregion_correct = 0
    span_correct = 0
...
def _compute_tp_fn_fp_mner(ps, ts,region_num):
    ...
    tp = span_correct
    fp = pred_sum - span_correct
    fn = supports - span_correct
    return tp,fp,fn

def _compute_tp_fn_fp_eeg(ps, ts,region_num):
    ...
    for k,v in ps.items():
        ...
        if span in ts:
            r,e = ts[span]
            if len(set(region_pred) & set(r)) != 0:
                if region_num not in set(r):
                    useful_correct +=1 
                else:
                    noregion_correct +=1
    ...
    tp = eeg_correct_num
    fp = pred_sum - eeg_correct_num
    fn = supports - eeg_correct_num
    return tp,fp,fn
